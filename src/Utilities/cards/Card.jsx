...
function Card({ eventData }) {
	...
	async function deleteEvent(id) {
		...
		try {
			const response = await axios.delete(
				`${baseUrl}/admin/get-event?eventId=${deleteId}`
			);
	...
	return (
		<>
			<div>
				<h1 className='text-md font-bold mb-4 mt-20'>
						...
						<div
							...
							<div className='relative w-full mt-5 pb-2 flex justify-between '>
								...
								<button
									...
									className=' flex justify-center items-center bg-[rgba(0,0,0,0.1)] rounded w-12'>
									<HiDotsVertical className='' />
								</button>

								<div
									className={`${
										active === item._id ? 'block' : 'hidden'
									}  absolute right-0 -top-20 flex flex-col bg-black text-white px-4 py-2 rounded`}>
